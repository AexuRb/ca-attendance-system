...
- 自定义 Excel 数据中心、操作日志、流式下载、自动保留清理与完整备份恢复
- 首次启动向导、响应式签到台和分角色后台
- Classic、Editorial、Spatial 三套全局界面，管理员在系统设置中切换
- 首页命令台、分级补全与按角色显示的快捷入口
...
| 部长 | 签到自动通过，审核成员记录，修改或删除本周成员及部长记录，查看并导出标准统计，管理维修事务；不能进入成员、培训或排班管理 |
| 会长 | 管理成员、培训、排班和维修，导入导出业务数据，补录及维护签到记录，执行备份；不能任命或维护管理员 |
| 管理员 | 拥有会长业务权限，并可维护管理员账号、查看日志、删除或恢复备份 |
...
- 远程管理入口：`127.0.0.1:8081`，仅供内网穿透程序连接
...
需要在外地管理时，可以让樱花穿透只转发本机 `127.0.0.1:8081`。该入口不展示签到台，只允许会长和管理员登录；远程初始化、公开签到、桌面控制，以及成员和部长账号均会由后端拒绝。`8080` 必须保持为主机本地入口，不能配置到穿透隧道中。

远程入口仍会接触成员和业务数据。若所选樱花隧道提供 HTTPS 或额外访问认证选项，建议启用；同时应设置高强度后台密码，并在不需要远程管理时关闭隧道。
...
仅在依赖与业务代码未变、明确沿用已完成测试时，可使用 `-SkipDependencyInstall -SkipTests` 重新封包；默认构建仍执行依赖安装和原有测试。
