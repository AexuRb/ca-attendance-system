...
用于计算机协会日常值班的本地网页系统，支持公开签到签退、签到页排班表、后台审核、成员管理、培训管理、维修事务与协议打印、批量导入、统计查询、数据中心、Excel 导出、数据库备份和管理员恢复备份。
...
`local-config.bat`、数据库备份、运行日志、依赖目录和构建缓存不会上传到 GitHub。请不要把真实数据库密码、管理员密码、内部资料或成员隐私数据提交到仓库。
